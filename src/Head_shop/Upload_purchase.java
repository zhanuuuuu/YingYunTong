...
@WebServlet(description = "提交采购", urlPatterns = { "/Upload_purchase" })
public class Upload_purchase extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			PreparedStatement past1 = conn.prepareStatement(
					"select SheetNo=" + new ReadConfig().getprop().getProperty("DataSource") + ".dbo.f_GenCaiGousheetno_manu('"+ String_Tool.DataBaseYear() + "','" + obj.getString("cStoreNo") + "')");
			...
			PreparedStatement past = conn.prepareStatement(
					"insert into WH_Stock(cStoreNo,cStoreName,dDate,cSheetNO,cSupplierNo,cSupplier,cOperatorNo,cOperator,cFillEmpNo,cFillEmp,dFillin,cFillinTime,iDays,cStockDptno,cStockDpt,fMoney,cWhNo,cWh,dDeadLine,cTime)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
