...
@WebServlet(description = "ÊÕÒøÃ÷Ï¸", urlPatterns = { "/Simple_cashier_reportDetail_forms" })
public class Simple_cashier_reportDetail_forms extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		LoggerUtil.info(UserNo);
		LoggerUtil.info(Start_Time);
		LoggerUtil.info(End_Time);
		CallableStatement c = null;
		try {
			c = conn.prepareCall("{Call p_Account_sheetList(?,?,?)}");
			c.setString(1, UserNo);
			c.setString(2, Start_Time);
			c.setString(3, End_Time);
			rs=c.executeQuery();
